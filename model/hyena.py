# This file is modified from https://github.com/HazyResearch/safari/blob/main/src/models/sequence/hyena.py and is separately licensed according to the following license:
"""
...
# disable compilation since it doesn't currently work with torch's fft since they use complex tensors, which dynamo doesn't support
import torch._dynamo
@torch._dynamo.disable
# reference convolution with residual connection
def fftconv_ref(u, k, D, dropout_mask=None, gelu=True, k_rev=None):
    seqlen = u.shape[-1]
    fft_size = 2 * seqlen
...
class HyenaOperator(nn.Module):
    ...
    def setup_filters(self, filter_cls, filter_args):   
        ...
        filter_cls = HyenaFilter# instantiate(registry.layer, filter_cls, partial=True)
    ...
    def forward(self, u, *args, **kwargs):
        ...
        *x, v = uc.split(self.d_model, dim=2)
        ...
        y = rearrange(v * x[0], 'b h v z l -> b (z l) (h v)', z=self.num_blocks, h=self.num_heads)
        y = self.out_proj(y)
...
class HyTention(nn.Module, model.core.IAttention):
    ...
    def forward(self, q:Tensor, k:Tensor, v:Tensor, recurrent_memory : Optional[Tensor] = None):
        B, H, T, Q = q.shape
        K = V = Q
        q = q.transpose(-1,-2).contiguous().view(B*H, Q, T)
        k = k.transpose(-1,-2).contiguous().view(B*H, K, T)
        v = v.transpose(-1,-2).contiguous().view(B*H, V, T)

        # FIXME - add in decay somehow?

        y = k * v
        y = F.pad(y, (T-1, 0)) # pads x so the convolution is causal
        y = q * self.kv_conv(y)

        y = y.transpose(-1,-2).contiguous().view(B, H, T, V)
