...
class RWKV7_0_AttentionSubLayer(model.core.TransformerLayerPart, model.interface.IAttentionSubLayer):
    def __init__(self):
        super().__init__()
        ...
        with torch.no_grad():
            ...
            D_GATE_LORA = 64
            self.time_gate_w1 = nn.Parameter(torch.empty(args.n_embd, D_GATE_LORA).uniform_(-0.01, 0.01))
            self.time_gate_w2 = nn.Parameter(torch.zeros(D_GATE_LORA, args.n_embd).uniform_(-0.01, 0.01))
    ...
    def forward(self, xq : Tensor, xk : Tensor, xv : Tensor, recurrent_memory : Optional[Tensor] = None):
        ...
        g = torch.tanh(gx @ self.time_gate_w1) @ self.time_gate_w2
