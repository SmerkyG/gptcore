...
def rwkv5_1_recurrent(s, r_in, k_in, v_in, w_in, u):
    L = r_in.size(-2)
    out = []
    for t in range(L):
...
def sanity_check():
    ...
    w = torch.rand(1,1,H,1).expand(B,T,H,K)
    u = torch.rand(  1,H,1).expand(  1,H,K)
    ...
    # recurrent
    out, _ = rwkv5_1_recurrent(s,r,k,v,w,u)
    print(out)

    # parallel
    out, _ = rwkv_inner(s,r,k,v,w,u,chunk_len=2)
    print(out)
